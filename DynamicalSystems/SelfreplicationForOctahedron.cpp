...
#include "SelfreplicationForOctahedron.hpp"
...
SelfreplicationForOctahedron::SelfreplicationForOctahedron(PeriodicBoundaryConditions &pbc_config,
...
SelfreplicationForOctahedron::~SelfreplicationForOctahedron()
{
...
void SelfreplicationForOctahedron::EvaluateRhsWithAllPairs(std::vector<Sphere> &system_state,
...
void SelfreplicationForOctahedron::EvaluateRhsWithLinkedList(std::vector<Sphere> &system_state,
                                                             const std::vector<Sphere> &k_prev,
                                                             std::vector<Sphere> &k_next,
                                                             Real k_coef,
                                                             Real dt_method_substep,
...
void SelfreplicationForOctahedron::EvaluateRhsWithVerletNeighborList(std::vector<Sphere> &system_state,
                                                                     const std::vector<Sphere> &k_prev,
                                                                     std::vector<Sphere> &k_next,
                                                                     Real k_coef,
                                                                     Real dt_method_substep,
...
void SelfreplicationForOctahedron::AddNoise(const std::vector<Sphere> &system_state,
                                            std::vector<Sphere> &derivative)
{
  static std::normal_distribution<Real> norm_dist01(0.0, 1.0);
  static const Real intensity = std::sqrt(2.0 * kSigma);
  for (int i = 0; i < system_state.size(); ++i)
  {
    derivative[i].SetPosition(Eigen::Vector3d::Zero());
    if (ParticleType::kColloidal == system_state[i].GetType())
    {
      derivative[i].SetVelocity(intensity * Eigen::Vector3d(norm_dist01(mersenne_twister_generator_),
                                                            norm_dist01(mersenne_twister_generator_),
                                                            norm_dist01(mersenne_twister_generator_)));
    } else if (ParticleType::kSolvent == system_state[i].GetType())
    {
      derivative[i].SetVelocity(intensity * Eigen::Vector3d(norm_dist01(mersenne_twister_generator_),
                                                            norm_dist01(mersenne_twister_generator_),
                                                            norm_dist01(mersenne_twister_generator_)));
...
Real SelfreplicationForOctahedron::SoftWeightingFunction(Real r_ij,
                                                         ParticleType type_i,
...
Real SelfreplicationForOctahedron::ConservativeInteractionForce(Real r_ij,
                                                                ParticleType type_i,
                                                                ParticleType type_j,
...
//      std::cout << "[" << static_cast<int>(subtype_i) << "][" << static_cast<int>(subtype_j) << "]" << std::endl;
    ...
    if (Sphere::IsFavorableInteraction(subtype_i, subtype_j))
    {
      ...
      if (r_ij <= r_cc_favorable)
      {
        ...
        force = 96.0 * interaction_coefficient * kEpsilon * r_ij_48 * (r_ij_48 - 1.0) / (r_ij);
      }
    } else
    {
      if (r_ij <= r_cc_unfavorable)
...
void SelfreplicationForOctahedron::AdjustNeighboringCellToPeriodicBoundaries(int &cell_x,
                                                                             int &cell_y,
                                                                             int &cell_z) const
{
  if (cell_x < 0)
...
void SelfreplicationForOctahedron::CalculateMaxDisplacement(const std::vector<Sphere> &derivative, Real dt)
{
